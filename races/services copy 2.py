...
def build_race_state(race_id, attempt=None):
    ...
    for ev in events:
        ...
        if ev.event_type == "start":
            ...
            b = state["boats"].get(ev.race_entry_id)
            if not b:
                continue
            ...
            state["race_time"] = max(
                state["race_time"],
                ev.race_seconds or 0
            )

            snapshot(state["race_time"], ev.race_entry_id)
    ...
    # -------------------------------------------------
    # FINAL POSITION CALCULATION (END STATE)
    # -------------------------------------------------
    boats = list(state["boats"].values())
